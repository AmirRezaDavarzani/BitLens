...
const CandlestickChart = ({
  ...
  const handlePriodChange = (newPeriod: Period) => {
  ...
  useEffect(() => {
    ...
    const convertedToSecondes = ohlcData.map(
        (item) => 
        [Math.floor(item[0] / 1000), item[1], item[2], item[3], item[4]] as OHLCData,
    );
    const converted = convertOHLCData(convertedToSecondes)
    ...
    <div id="candlestick-chart">
      <div className="chart-header">
        ...
        <div className="button-group">
          <span className="text-sm mx-2 font-medium to-purple-100/50">
            Period:
          </span>
          {PERIOD_BUTTONS.map(({ value, label }) => (
            <button
              ...
              onClick={() => handlePriodChange(value)}
              disabled={loading}
            >
              {label}
            </button>
